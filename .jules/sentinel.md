...
When extending Python's http.server.SimpleHTTPRequestHandler, avoid blocklists as they are inherently flawed against path traversal. Instead, implement a strict allowlist for both do_GET and do_HEAD by normalizing paths with self.translate_path(self.path), verifying they reside within the base directory (os.path.abspath(os.getcwd())), and explicitly permitting only required files and directories using os.path.relpath.
