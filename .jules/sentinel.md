...
The backend () identifies user sessions securely by checking for a `session_id` in the `Cookie` header. If missing, it generates a new UUID (`uuid4()`) and returns it via a `Set-Cookie` header (`HttpOnly`, `Path=/`) to enforce per-user state and prevent global data sharing. When testing endpoints that require cookie persistence (e.g., `/api/sync` session IDs) in Python scripts, use `http.cookiejar.CookieJar()` in combination with `urllib.request.build_opener` to automatically capture and send cookies across sequential requests.
The backend (`app.py`) identifies user sessions securely by checking for a `session_id` in the `Cookie` header. If missing, it generates a new UUID (`uuid4()`) and returns it via a `Set-Cookie` header (`HttpOnly`, `Path=/`) to enforce per-user state and prevent global data sharing. When testing endpoints that require cookie persistence (e.g., `/api/sync` session IDs) in Python scripts, use `http.cookiejar.CookieJar()` in combination with `urllib.request.build_opener` to automatically capture and send cookies across sequential requests.
